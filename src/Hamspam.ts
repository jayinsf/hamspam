...
//creates dynamically email object and view object based on the current page's url
for (let conf of Config.getInstance().getConfigurationOf('email')) {
  ...
  if (anchor.hostname === conf.hostname) {
    ...
    //removes all old indicators before drawing new
    view.clear();
